...
$app->get('/concerts_testing/import/browse/{query}', function($request, $response, $args) {
	...
	$query = $args['query'];
	$stmt = $this->db->prepare("SELECT * FROM testing WHERE query = '{$query}'");
	$stmt->execute();
	$videoList = $stmt->fetchAll(PDO::FETCH_ASSOC);
	// $response-write(var_dump($videoList));
...
$app->get('/concerts_testing/update_video_quality_score/{video_id}/{score}', function($request, $response, $args) {
	...
	$stmt = $this->db->prepare("UPDATE testing SET quality_score = '{$args['score']}' WHERE video_id = '{$args['video_id']}'");
	$stmt->execute();
});
...
$app->get('/concerts_testing/update_video_relevance_score/{video_id}/{score}', function($request, $response, $args) {
	...
	$stmt = $this->db->prepare("UPDATE testing SET relevance_score = '{$args['score']}' WHERE video_id = '{$args['video_id']}'");
	$stmt->execute();
});
...
$app->get('/concerts_testing/update_video_relevance_null_values/{query}', function($request, $response, $args) {
	...
	$stmt = $this->db->prepare("UPDATE testing SET relevance_score = 'bad' WHERE query = '{$args['query']}' AND relevance_score is null");
	$stmt->execute();
});